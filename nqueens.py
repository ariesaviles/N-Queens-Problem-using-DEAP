...
#  Draw Board
# =========================
def display_positional_grid(queen_positions: List[int]):
    # set dimensions to specified number of
    dimension = len(queen_positions)

    # construct board using pandas
    board = pandas.DataFrame("", index = range(1, dimension + 1), columns = range(1, dimension + 1))

    # draw conflicts within range to n
    for x in range(dimension):
        x_row, x_column = queen_positions[x] // dimension, queen_positions[x] % dimension
        for y in range(x + 1, dimension):
            y_row, y_column = queen_positions[y] // dimension, queen_positions[y] % dimension
...
#  N = 16
# =========================
duration, log, fittest_individual_p5 = evolution(
    individual_generator=lambda: random.choices(range(25**2), k=25),
...
# Render
print(f"Positions: {fittest_individual_p5}")
print(f"Duplicate queens: {len(fittest_individual_p5) - len(set(fittest_individual_p5))}")
print(f"Fitness: {abs(fittest_individual_p5.fitness.values[0])}")
display_positional_grid(fittest_individual_p5)